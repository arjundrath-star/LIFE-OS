...
2. docs/plans/pokemon-ops/phases/PHASE-3.md (your spec)
...
commit + push → annotated tag pokemon-ops/phase-3 → update the PROGRESS.md current-state
block → agent-event complete.

This phase is ultracode-recommended: the rules engine's golden fixtures must contain
hand-computed arithmetic; have the adversarial pass re-derive the expected values
