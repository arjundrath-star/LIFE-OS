...
2. docs/plans/pokemon-ops/phases/PHASE-4.md (your spec)
...
commit + push → annotated tag pokemon-ops/phase-4 → update the PROGRESS.md current-state
block → agent-event complete.

This phase is ultracode-recommended (E2E flakiness). Remember the repo's Puppeteer
lesson: page.evaluate(el.click()) for row handlers.
