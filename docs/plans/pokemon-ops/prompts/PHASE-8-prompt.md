...
2. docs/plans/pokemon-ops/phases/PHASE-8.md (your spec)
...
commit + push → annotated tag pokemon-ops/phase-8 → update the PROGRESS.md current-state
block → agent-event complete.

PRECONDITION CHECK FIRST: SQS queue configured and validated in Nayax Core per the spec.
Missing → halt with a PROGRESS.md entry.
