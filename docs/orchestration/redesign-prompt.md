...
## Read first
- The app is at `~/rathworkspace` (Next.js, runs as the systemd `rathworkspace` service bound to loopback, served at https://rathworkspace.cloud through a cloudflared tunnel). It is LIVE and behind the Google gate. Code repo: github.com/arjundrath-star/LIFE-OS.
...
- A persistent **collapsible left nav rail**: collapsed = icon strip (~56px), expanded = labeled (~220px), state persisted (localStorage). **Flat, all items top-level** (an explicit design choice), in this order: Home, Agents, Email, Calendar, Vending, Ad Agency, School, Health, Connections, Projects, Terminal, Files, Accounts.
- **The rail is itself a live instrument:** each item carries a small live status indicator driven by the existing WebSocket — Agents shows a pulsing green/amber/red dot, Email shows the unread count, School shows the days-until-term-start countdown, Connections shows an amber dot if any connection is broken. Even collapsed to icons, the rail reads as an ambient health strip.
...
Home is the curated glance, NOT a launcher and NOT everything. It shows:
...
- **Vending Ops** `/vending` (the big one): hero = revenue today / this week / MTD (bank feed when connected, honest "connect the bank feed" stub until then). Sections: Machines (each a card: location, stock level, last refill, needs-refill flag), Deal pipeline as stages (lead → venue said yes → machine being placed → live), Outreach-this-week counter (from the identified inbox) with reply signal, and a small map/placement view of machines.
- **Ad Agency** `/ad-agency`: hero = the portfolio (the videos: thumbnail, play, the live website link). Sections: Higgsfield credit balance (live via the `higgsfield account status` CLI) + a "generate a video" launch link + recent generations (`higgsfield generate list`), and the outreach pipeline (leads → sent → replies).
- **School** `/school`: hero = the big countdown to the term start date (it owns the page). Now (pre-term): a "set up before term" checklist + the calendar feed placeholder. Once classes start: today's classes, assignments due, the week, all from the university calendar feed.
- **Agents / Connections / Email / Calendar / Health / Terminal / Files / Accounts**: each gets its own full page with the existing content, given proper space (e.g. Terminal and Files render full-height, not a tiny 46x14 box). **Projects** `/projects` is the hub listing all vault-derived projects (Portable Charging, Ad Agency, School, etc.), each linking to a project view.
...
- Honest-UI everywhere: explicit "connect me / no data" states for anything unwired (Whoop, the bank feed). Never fake a metric.
...
- Secrets stay server-side in the env file loaded by `lib/secrets.ts`; ttyd/filebrowser stay bound to localhost behind the gate; the Google allowlist gate stays enforced on every route (including the new ones: protect the whole `(dash)` group).
...
When you reach the definition of done, OR you hit a blocker you cannot resolve, email the operator a status summary before you stop:
```
gws gmail +send --to <operator-email> --subject "RATHWORKSPACE v2 — done" --body "<summary>"
```
The default `gws` account is already authed. Summary covers: what got restructured and verified, anything that regressed or is still stubbed (Whoop, the bank feed), any blockers, and the live URL https://rathworkspace.cloud. Plain founder voice, no em dashes, no hype words. This is mandatory. The operator detaches from the session and will not be watching; the email is how they know it finished.
