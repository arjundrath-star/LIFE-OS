...
import type { SternMemo, SternReminder } from "@/lib/stern-types";
import { sternSnapshot } from "./snapshot";
import { nyDayBounds, nyDateKey, localDateKey, nyClock, nyWallTime, dayWindowSql, dayWindowParams } from "./time";
import { reminderChats, replyOwed, thankYouOwed, reminderPrograms, activeInterview } from "./reminders";
import { notificationText, queueReminder, reminderMeta, reminderRow, changeReminder, reminderMessage } from "./reminder-store";
...
export function buildMemo(date: Date = new Date()): SternMemo {
  ...
  const todayChats = chats.filter(ch => ch.state === "scheduled" && ch.scheduled_at && localDateKey(ch.scheduled_at) === today.dateKey);
  ...
  return { date: today.dateKey, subject: `Daily memo, ${today.dateKey}`, email: notificationText(email), imessage: notificationText([...(concise.length ? concise : ["No urgent Stern items today."]), "Full memo in email."].join("\n")) };
}
...
export async function sendMemo(date = new Date(), options: SendOptions = {}) {
  ...
  const dateKey = nyDateKey(date);
  const existing = getDb().prepare(`SELECT * FROM stern_reminders WHERE rule_key='memo' AND json_valid(message)
    AND json_extract(message,'$.key') IN (?,?) ORDER BY id`).all(`memo:email:${dateKey}`, `memo:imessage:${dateKey}`) as SternReminder[];
  const canAttempt = (row: SternReminder) => ["pending", "snoozed"].includes(row.delivery_status) || (!dryRun && row.error === "dry-run" && row.delivery_status === "skipped");
  if (existing.length === 2 && existing.every(row => !canAttempt(row))) return { skipped: true, reason: "already-attempted", deliveries: existing };
  ...
  const currentDeliveries = deliveries.map(d => ({ ...reminderRow(d.id), attempted: d.attempted, ...(!d.attempted ? { reason: "already-attempted" } : {}) }));
  if (!dryRun && currentDeliveries.every(d => d.delivery_status === "sent")) {
    getDb().transaction(() => writeNotificationSetting("stern.memo_last_date", memo.date, audit)).immediate();
  }
  return { memo, deliveries: currentDeliveries, skipped: !deliveries.some(d => d.attempted) };
}
