...
// Task and assignment writes go through WP4's domain modules (lib/stern/tasks.ts, lib/stern/classes.ts)
// so validation, dedupe keys, and audit rows are shared with the UI. Repeat messages patch the
// existing row's editable fields instead of creating a duplicate.
export function upsertAutomationTask(fields: Row & { dedupe_key: string; title: string }, audit: AuditMeta): number {
  const { dedupe_key, source: _source, ...rest } = fields as Row & { dedupe_key: string; source?: string };
  const existing = getDb().prepare("SELECT id FROM stern_tasks WHERE dedupe_key=?").get(dedupe_key) as { id: number } | undefined;
  if (existing) {
    const patchable: Row = {};
    for (const k of TASK_EDITABLE) if (rest[k] !== undefined) patchable[k] = rest[k];
    if (Object.keys(patchable).length) updateTask(existing.id, patchable, audit);
    return existing.id;
  }
  return createTask({ source: "auto", ...rest, dedupe_key }, audit).id;
}
function assignment(cls: EmailClassification, message: SternEmailMessage, courseId: number, audit: AuditMeta) {
  ...
  const course = row<{ code: string }>("course", courseId);
  const saved = upsertAssignmentFromEmail(course.code, {
    title: input.title,
    kind: input.kind || "other",
    ...(input.due_at ? { dueAt: input.due_at } : {}),
    ...(input.points_possible != null ? { points: input.points_possible } : {}),
  ...
  if (cls.category === "brightspace_grade") {
    ...
    gradeAssignment(saved.id, Number(grade[1]), Number(grade[2]), audit);
  }
  return saved.id;
}
