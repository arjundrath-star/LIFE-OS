...
export function automationDetails() {
  ...
  return {
    ...
    suggestions: (db.prepare("SELECT * FROM stern_suggestions WHERE state='pending' ORDER BY id DESC LIMIT 100").all().reverse() as Omit<SternSuggestion,"summary">[]).map(s => ({...s,summary:suggestionSummary(s)})),
...
export async function automationSnapshot(email = ""): Promise<SternAutomationResponse> {
  ...
  return { ...automationDetails(), updatedAt: nowIso(), connectHref: missing ? `/api/google/connect?set=stern&target=stern&login_hint=${encodeURIComponent(email)}` : "" };
}
