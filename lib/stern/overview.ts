...
export function todaySchedule(now = new Date()): SternScheduleItem[] {
  ...
  const events = db.prepare(`SELECT e.*, COALESCE(NULLIF(ch.club_id,0),p.club_id,0) club_id
    ...
    LEFT JOIN stern_programs p ON p.id=e.program_id WHERE ${dayWindowSql('e.start_at')}
...
export function needsYou(): SternNeed[] {
  ...
  const chats=db.prepare(`SELECT c.id,c.person_id,c.state,c.reply_needs_me,c.reply_at,c.occurred_at,p.display_name
    ...
    ((c.reply_needs_me=1 AND c.state NOT IN ('done','thank_you_sent','declined','no_reply')) OR (c.state='done' AND c.thank_you_sent_at='')) ORDER BY c.id`).all() as {id:number;person_id:number;state:string;reply_at:string;occurred_at:string;display_name:string}[];
  ...
  const drafts=db.prepare(`SELECT d.id,d.person_id,d.subject,d.created_at,p.display_name FROM stern_drafts d
    LEFT JOIN people p ON p.id=d.person_id WHERE d.state='generated' AND (p.id IS NULL OR p.archived=0) ORDER BY d.id`).all() as {id:number;person_id:number;subject:string;created_at:string;display_name:string}[];
  ...
  const suggestions=db.prepare("SELECT id,evidence_subject,created_at FROM stern_suggestions WHERE state='pending' ORDER BY id").all() as {id:number;evidence_subject:string;created_at:string}[];
