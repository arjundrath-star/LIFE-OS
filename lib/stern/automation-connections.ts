...
export async function automationConnections(): Promise<SternConnectionCard[]> {
  ...
  const accounts=db.prepare('SELECT email,enabled,last_error FROM google_accounts ORDER BY enabled DESC,added_at DESC').all() as {email:string;enabled:number;last_error:string}[];
  ...
  return specs.map(([id,label,target])=>{
    ...
    const a=target?accounts.find(a=> target==='stern'?/@stern\.nyu\.edu$/i.test(a.email):target==='nyu'?/@nyu\.edu$/i.test(a.email):/@gmail\.com$/i.test(a.email)):undefined;
    ...
    return {id,label,state,detail,account:a?.email||'',scopes:a?accountScopes(a.email).filter(s=>!['openid','email','profile'].includes(s)).map(s=>s.split('/').pop()||s):[],lastScan:scan?.last_checked||cached?.last_checked||'',
