...
function ReadOnlyCrm({ source, query }: { source: CrmSheetSource; query: string }) {
  const { data, loading } = useApi<CrmSheetResult>(`/api/business/crm-sheets?source=${encodeURIComponent(source.id)}`);
  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    return (data?.rows || []).filter(row => !q || `${row.venue} ${row.category} ${row.cityRegion} ${row.contact} ${row.status} ${row.nextAction} ${row.notesSummary}`.toLowerCase().includes(q));
  }, [data, query]);
  if (!data) return <div className="business-empty" role={loading ? "status" : "alert"}><h2>{loading ? `Loading ${source.label}` : "Server error"}</h2><p>{loading ? "Reading the server-side source." : "The authenticated source endpoint did not return data. Try again shortly."}</p></div>;
  ...
  return <>
    <div className="business-facts crm-sheet-kpis">
      ...
      <div className="business-fact"><span>Visible</span><strong>{rows.length}</strong><small>current search</small></div>
    ...
    <section className="business-section"><header><h2>{source.label}</h2><span className="business-source">{rows.length} of {data.total} shown</span></header><div className="business-section-body !p-0">
      {rows.length === 0 ? <div className="p-6 text-sm text-txt-muted">{query ? "No rows match this search." : "This source has no rows."}</div> : <div className="business-table-wrap"><table className="business-ops-table"><thead><tr>{data.fields.slice(0, 7).map(f => <th key={f.key}>{f.label}</th>)}</tr></thead><tbody>{rows.map(row => <tr key={row.id}><td><strong>{row.venue}</strong>{row.notesSummary && <small>{row.notesSummary}</small>}</td><td>{row.category || "—"}</td><td>{row.cityRegion || "—"}</td><td>{row.contact || "—"}</td><td>{row.status || "—"}</td><td>{row.lastTouch || "—"}</td><td>{row.nextAction || "—"}</td></tr>)}</tbody></table></div>}
    </div></section>
  </>;
}
...
export default function CrmWorkspace() {
  ...
  return <div className="business-page crm-connected">
    ...
    <div className="business-source-tabs" role="tablist" aria-label="CRM source">
      {sources.map(source => <button key={source.id} role="tab" aria-selected={active === source.id} className={active === source.id ? "active" : ""} onClick={() => setActive(source.id)}><Database /> <span>{source.label}</span><small>{source.editable ? "Editable" : source.available ? `${source.count ?? "—"} · Read-only` : "Unavailable"}</small></button>)}
    </div>
    ...
    {active === "pokemon-crm" ? <PokemonCrmWorkspace query={query} setQuery={setQuery} /> : <ReadOnlyCrm source={selected} query={query} />}
  </div>;
}
