...
import type { PkProduct, PkSourceProductCoverage, PkSourceProductCurrent } from "@/lib/pokemon-ops/types";

type SourcingOps = { offers:any[]; benchmarks:any[]; vendors:any[]; sourceProducts:PkSourceProductCurrent[]; sourceProductCoverage:PkSourceProductCoverage[]; local:{provenance:any;spots:any[]}; discord:any[]; connector:any; monitors:any[] };
...
function SourceProductTargets({rows,coverage}:{rows:PkSourceProductCurrent[];coverage:PkSourceProductCoverage[]}) {
  ...
  return <>
    ...
    <BusinessSection title="Box and sealed-product buy targets" note={<SourceStamp>append-only dated snapshots · pack economics only</SourceStamp>}>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <select id="source-product-set" className="rounded-md border border-white/20 bg-surface px-3 py-2 text-sm text-txt" value={setFilter} onChange={event=>setSetFilter(event.target.value)}>
      ...
      {visible.length===0?<p className="text-sm text-txt-muted">No dated sealed-product values are available for this filter.</p>:<div className="business-table-wrap"><table className="business-ops-table" data-testid="source-product-targets"><thead><tr><th>Exact product</th><th>Packs</th><th>TCG sealed market</th><th>Pack benchmarks</th><th>Low buy</th><th>Medium buy</th><th>High buy</th><th>Snapshot</th></tr></thead><tbody>{visible.map(row=><tr key={row.source_product_id}><td><a className="text-accent underline" href={row.tcgplayer_url} rel="noreferrer" target="_blank"><strong>{row.name}</strong></a><small>{row.set_name} · {row.form.replaceAll("_"," ")}</small></td><td><strong>{row.pack_count}</strong><small title={row.pack_count_note}>verified count</small></td><td><strong>{money(row.tcg_market_cents)}</strong><small>{row.tcg_market_ppp_cents==null?"No sealed market":`${money(row.tcg_market_ppp_cents)} / pack`}</small></td><td><strong>{money(row.benchmark_ppp_cents)} / pack</strong><small>TCG {money(row.pack_tcg_cents)} · Carddistro {money(row.carddistro_cents)}</small></td><td><strong>{money(row.low_total_cents)}</strong><small>{money(row.low_ppp_cents)} / pack</small></td><td><strong>{money(row.medium_total_cents)}</strong><small>{money(row.medium_ppp_cents)} / pack</small></td><td><strong>{money(row.high_total_cents)}</strong><small>{money(row.high_ppp_cents)} / pack</small></td><td><strong>{formatDate(row.observed_date)}</strong><small>historical row preserved</small></td></tr>)}</tbody></table></div>}
  ...
  return <PokemonDataBoundary><BusinessPage title={inventory?"Inventory":"Sourcing"} description={inventory?"Operate available, allocated, and in-transit stock first; review history or record activity in dedicated views.":"Compare actionable offers with market evidence, supplier decisions, local call-ahead routes, and connector health."} actions={snapshot&&<SourceStamp>Pokemon Ops · <Fresh value={snapshot.asOf}/></SourceStamp>}>
    {!snapshot?<div className="business-empty" role={error?"alert":"status"}><h2>{error?"Could not load Pokemon Ops":"Loading Pokemon Ops"}</h2><p>{error||"Reading the authenticated operational snapshot."}</p>{error&&<button onClick={refetch}>Retry</button>}</div>:inventory?<>
      ...
      {!sourcing?<div className="business-empty" role={sourcingError?"alert":"status"}><h2>{sourcingError?"Could not load sourcing evidence":"Loading sourcing evidence"}</h2><p>{sourcingError||"Grouping observations, recommendations, and source health."}</p>{sourcingError&&<button onClick={refetchSourcing}>Retry</button>}</div>:<>
        {sourcingView==="Box Targets"&&<SourceProductTargets rows={sourcing.sourceProducts??[]} coverage={sourcing.sourceProductCoverage??[]}/>}
