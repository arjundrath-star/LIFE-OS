...
export default function LocationsWorkspace() {
  ...
  const { data: service } = useApi<any>("/api/business/service");
  ...
  return <PokemonDataBoundary><BusinessPage title="Locations" description="Real placement opportunities and machine operations from the existing Vending and Pokemon Ops stores." actions={<SourceStamp>vending SQLite + live channel</SourceStamp>}>
    {!snapshot ? <div className="business-empty"><h2>Loading locations</h2><p>Reading the authenticated vending snapshot.</p></div> : <>
      ...
      <BusinessSection title="Machine service priority" note={<div className="flex flex-wrap gap-2"><input aria-label="Machine name" placeholder="Machine name" value={machine} onChange={(e) => setMachine(e.target.value)} className="h-11 border border-border bg-base px-3 text-sm" /><input aria-label="Machine location" placeholder="Location" value={location} onChange={(e) => setLocation(e.target.value)} className="h-11 border border-border bg-base px-3 text-sm" /><button className="h-11 bg-accent px-4 text-sm font-semibold text-white" onClick={addMachine}>Add machine</button></div>}>
        {(service?.machines ?? []).length === 0 ? <p className="text-sm text-txt-muted">{service?"No machines recorded yet.":"Loading service priorities…"}</p> : <div className="business-list">{service.machines.map((item: any) => <article key={item.id} className="fleet-row"><div><span className={`machine-condition ${item.condition}`}>{item.condition.replaceAll("_"," ")}</span><strong>{item.name}{item.asset_code?` · ${item.asset_code}`:""}</strong><p>{item.location || "Unplaced"} · {item.calculated_stock}/{item.capacity} calculated stock</p><small>{item.latest_verified_at?`Verified at ${new Date(item.latest_verified_at).toLocaleString()}`:"No physical count"} · {item.last_service?`last service ${timeAgo(item.last_service.completed_at)}`:"no service visit"} · {item.open_issues.length} open issue{item.open_issues.length===1?"":"s"}</small></div><div className="fleet-actions"><Link href={`/business/locations/${item.id}`}>View machine</Link><Link href={`/business/locations/${item.id}/service`}>Start service</Link></div></article>)}</div>}
      </BusinessSection>
