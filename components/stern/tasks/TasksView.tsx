...
export function TasksView(){
  const {data,error,busy,mutate}=useSternArea('tasks');const [domains,setDomains]=useState<TaskDomain[]>([]),[grouped,setGrouped]=useState(true),[editing,setEditing]=useState<SternTask|null>(null),[history,setHistory]=useState(false);
  ...
  return <SternPage title="Tasks" testId="stern-tasks-view" actions={<><div className="stern-domain-chips" data-testid="stern-tasks-domains"><button data-testid="stern-tasks-domain-all" aria-pressed={!domains.length} onClick={()=>setDomains([])}>All</button>{TASK_DOMAINS.map(domain=><button key={domain} data-testid={`stern-tasks-domain-${domain}`} aria-pressed={domains.includes(domain)} onClick={()=>setDomains(v=>v.includes(domain)?v.filter(d=>d!==domain):[...v,domain])}>{TASK_DOMAIN_LABELS[domain]}</button>)}</div><label className="stern-group-switch"><input data-testid="stern-tasks-group" type="checkbox" checked={grouped} onChange={e=>setGrouped(e.target.checked)}/>Group by due</label></>}>
    {error&&<p role="alert" className="stern-recruiting-error">{error}</p>}
    {!data?<SkeletonRows/>:<div className="stern-tasks-columns"><div className="stern-task-groups" data-testid="stern-tasks-list">
      {groups.map(g=><section key={g.key} data-testid={`stern-tasks-group-${g.key}`}><h2>{g.title} <span className="stern-mono">{g.rows.length}</span></h2><div className="stern-row-surface">{g.rows.length?g.rows.map(taskRow):<EmptyState title={`No ${g.title.toLowerCase()} tasks`} hint={g.key==='all'?'Add a task to get started.':undefined}/>}</div></section>)}
      ...
      <button className="stern-text-button" data-testid="stern-tasks-history-toggle" onClick={()=>setHistory(v=>!v)}>{history?'Hide':'Show'} completed and dropped tasks</button>
...
function TaskComposer({task,data,busy,save,cancel}:{task:SternTask|null;data:TasksSnapshot;busy:boolean;save:(body:Record<string,unknown>)=>Promise<void>;cancel:()=>void}){
  ...
  return <form className="stern-task-composer stern-form-stack stern-row-surface" data-testid="stern-task-composer" onSubmit={submit}><h2>{task?'Edit task':'Add task'}</h2>
    ...
    {task&&<><Button type="button" data-testid="stern-task-cancel" onClick={cancel}>Cancel</Button><Button type="button" disabled={busy} data-testid="stern-task-drop" onClick={()=>void save({action:task.status==='dropped'?'task.reopen':'task.drop',id:task.id}).catch(()=>{})}>{task.status==='dropped'?'Reopen task':'Drop task'}</Button></>}
  </form>;
}
