...
export function NetworkTable() {
  ...
  return <SternPage title="Network" subtitle={<span className="stern-mono">{data?.counts.total ?? "—"} people</span>} testId="stern-network" actions={<><a className="stern-btn" href={`/api/stern/network?${query}&export=csv`} data-testid="stern-network-export-csv">Export CSV</a><a className="stern-btn" href={`/api/stern/network?${query}&export=json`} data-testid="stern-network-export-json">Export JSON</a><Button className="stern-btn primary" onClick={quickAdd} data-testid="stern-network-add"><Plus size={14} />Quick add</Button></>}>
    ...
    {loading ? <SkeletonRows rows={8} /> : data && <div className="stern-network-table-wrap">
      <table className="stern-network-table" data-testid="stern-network-table"><thead><tr>{["Name", "Affiliation", "Relationship", "Strength", "Status", "Last contact", "Next action", "Contact"].map(h => <th key={h}>{h}</th>)}</tr></thead><tbody>
        {data.people.map(p => <tr key={p.id} data-testid="stern-network-row" data-person-id={p.id} onClick={() => selectPerson(p.id)}>
          ...
          <td className="stern-mono" title={p.last_contact_at}>{p.last_contact_at ? timeAgo(p.last_contact_at) : "—"}</td><td>{p.next_action || "—"}</td>
