...
export function NetworkTable() {
  ...
  return <SternPage title="Network" subtitle={<span className="stern-mono">{data ? data.counts.total : "…"} people</span>} testId="stern-network" actions={<><a className="stern-btn" href={`/api/stern/network?${query}&export=csv`} data-testid="stern-network-export-csv">Export CSV</a><a className="stern-btn" href={`/api/stern/network?${query}&export=json`} data-testid="stern-network-export-json">Export JSON</a><button type="button" className="stern-btn primary" onClick={quickAdd} data-testid="stern-network-add"><Plus size={14} />Quick add</button></>}>
