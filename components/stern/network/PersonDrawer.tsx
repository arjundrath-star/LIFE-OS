  ...
  return <NetworkDialog open onClose={() => { void close(); }} title={person?.display_name || "Person"} testId="stern-person-drawer">
    {loading && !person ? <SkeletonRows rows={8} /> : person ? <>
      ...
      <div className="stern-person-body">
        ...
        <section><h3>Coffee chat</h3><div data-testid="stern-person-coffee-chats">{!person.coffeeChats.length && <p className="stern-network-muted">No coffee chats yet.</p>}{person.coffeeChats.map(chat => <div key={chat.id} className="stern-person-coffee"><span data-component="CoffeeChatChip"><StatusChip value={coffeeChatPhase(chat)} /></span><dl><dt>Requested</dt><dd>{chat.requested_at ? new Date(chat.requested_at).toLocaleString() : "Not requested"}</dd><dt>Scheduled</dt><dd>{chat.scheduled_at ? new Date(chat.scheduled_at).toLocaleString() : "Not scheduled"}</dd><dt>Location</dt><dd>{chat.location || "Not added"}</dd><dt>Thank-you</dt><dd>{chat.thank_you_sent_at ? new Date(chat.thank_you_sent_at).toLocaleString() : "Not sent"}</dd></dl>{chat.takeaways && <p>{chat.takeaways}</p>}</div>)}</div>
