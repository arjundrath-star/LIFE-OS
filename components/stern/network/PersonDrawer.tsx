...
import { HOW_MET_LABELS, PERSON_STATUSES, RELATIONSHIP_TYPES, statusLabel, type NetworkResponse, type PersonDetail, type SternSnapshot } from "@/lib/stern-types";
import { timeAgo } from "@/lib/time";
import { Field, NetworkDialog, Toggle, networkAction, statusAllowed } from "./shared";
      ...
      <div className="stern-person-body">
        ...
        <section><h3>Affiliations</h3><div data-testid="stern-person-affiliations">{!person.affiliations.length && <p className="stern-network-muted">No affiliations yet. Add a club or organization.</p>}{person.affiliations.map(a => <div key={a.id} className="stern-person-affiliation"><span className="stern-network-affiliation-chip">{a.club_name || a.org}{a.role && ` · ${a.role}`}</span>{a.is_eboard === 1 && <b className="stern-network-eboard">E-board</b>}<Toggle label="Relevant for recruiting" checked={!!a.relevant_for_recruiting} disabled={busy} onChange={value => void mutate({ action: "affiliation.update", id: a.id, patch: { relevantForRecruiting: value } })} testId={`stern-person-relevant-${a.id}`} /><button className="stern-network-text-button" disabled={busy} onClick={() => void mutate({ action: "affiliation.remove", id: a.id })} data-testid={`stern-person-affiliation-remove-${a.id}`}>Remove</button></div>)}</div>
          <form className="stern-network-form" onSubmit={async e => { e.preventDefault(); const element = e.currentTarget; const f = new FormData(element); if (await mutate({ action: "affiliation.add", personId: id, affiliation: { clubId: Number(f.get("club")), org: f.get("org"), role: f.get("role"), isEboard: f.has("eboard"), relevantForRecruiting: f.has("relevant") } })) element.reset(); }}>
            ...
            <Button className="stern-btn" disabled={busy} data-testid="stern-person-affiliation-add"><Plus size={14} />Add affiliation</Button>
          </form>
        </section>
        <section><h3>Coffee chat</h3><div data-testid="stern-person-coffee-chats">{!person.coffeeChats.length && <p className="stern-network-muted">No coffee chats yet.</p>}{person.coffeeChats.map(chat => <div key={chat.id} className="stern-person-coffee"><span data-component="CoffeeChatChip"><StatusChip value={chat.state} /></span><dl><dt>Requested</dt><dd>{chat.requested_at ? new Date(chat.requested_at).toLocaleString() : "Not requested"}</dd><dt>Scheduled</dt><dd>{chat.scheduled_at ? new Date(chat.scheduled_at).toLocaleString() : "Not scheduled"}</dd><dt>Location</dt><dd>{chat.location || "Not added"}</dd><dt>Thank-you</dt><dd>{chat.thank_you_sent_at ? new Date(chat.thank_you_sent_at).toLocaleString() : "Not sent"}</dd></dl>{chat.takeaways && <p>{chat.takeaways}</p>}</div>)}</div>
          <div data-component="DraftPanel" className="stern-person-drafts" data-testid="stern-person-drafts">{!person.drafts.length && <p>No drafts yet</p>}{person.drafts.map(d => <article key={d.id}><div className="stern-network-inline"><strong>{statusLabel(d.kind)} draft</strong><StatusChip value={d.state} /></div><b>{d.subject}</b><p>{d.body}</p><Button className="stern-btn" onClick={() => void copy(d.body)} data-testid={`stern-person-draft-copy-${d.id}`}><Copy size={14} />Copy</Button></article>)}</div>
        </section>
        ...
        <section><h3>Touchpoints</h3><ol data-component="TouchpointTimeline" data-testid="stern-person-touchpoints" className="stern-person-timeline">{!person.touchpoints.length && <li className="stern-network-muted">No touchpoints yet. Add a note to record a conversation.</li>}{person.touchpoints.map(t => <li key={t.id}><div><strong>{t.summary || statusLabel(t.kind)}</strong><time className="stern-mono" dateTime={t.occurred_at} title={t.occurred_at}>{timeAgo(t.occurred_at)}</time><SourceBadge source={t.source} /></div>{t.detail && <p>{t.detail}</p>}</li>)}</ol>
          <form className="stern-network-inline" onSubmit={async e => { e.preventDefault(); const form = e.currentTarget; const summary = new FormData(form).get("summary"); if (await mutate({ action: "touchpoint.add", personId: id, kind: "note", summary })) form.reset(); }}><input className="stern-input" name="summary" required placeholder="Add a touchpoint note" aria-label="Touchpoint note" data-testid="stern-person-touchpoint-note" /><Button className="stern-btn" disabled={busy} data-testid="stern-person-touchpoint-add">Add</Button></form>
        </section>
      </div>
      <footer className="stern-person-footer"><select className="stern-select" aria-label="Set status" value={person.status} disabled={busy} onChange={e => void mutate({ action: "person.set_status", id, status: e.target.value })} data-testid="stern-person-status">{PERSON_STATUSES.map(s => <option key={s} value={s} disabled={!statusAllowed(person.status, s)}>{statusLabel(s)}</option>)}</select><Button className="stern-btn" data-testid="stern-person-add-task" onClick={() => window.dispatchEvent(new CustomEvent("stern:quick-add", { detail: { segment: "Task", personId: id } }))}>Add task</Button><Button className="stern-btn ghost" disabled={busy || !!person.archived} data-testid="stern-person-archive" onClick={async () => { try { await saveNotes(); if (await mutate({ action: "person.archive", id })) onClose(); } catch { setNotice("Save notes before archiving."); } }}>{person.archived ? "Archived" : "Archive"}</Button></footer>
    </> : <p role="alert" className="stern-person-body">{error || "Person not found"}</p>}
  </NetworkDialog>;
}
