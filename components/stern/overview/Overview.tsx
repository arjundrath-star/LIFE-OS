...
export function Overview() {
  ...
  const action=useAutomationAction(r=>setSaved(r.snapshot));
  ...
  return <SternPage title="Today," subtitle={<span className="stern-mono">{date}</span>} testId="stern-overview">
    ...
    <div className="stern-overview-grid">
      ...
      <SternSection title="Needs you" className="stern-overview-needs" flush testId="stern-overview-needs"><div data-testid="stern-needs-list">{!data?<SkeletonRows/>:!data.needsYou.length?<EmptyState icon={<Inbox/>} title="Nothing waiting on you" hint="Replies, thank-yous, drafts, and suggestions appear here."/>:data.needsYou.map(n=><div className="stern-need-row" key={n.key}><i data-kind={n.kind}/><span>{n.title}</span><Link data-testid={`stern-need-${n.key}`} href={n.href}>{n.actionLabel}</Link></div>)}</div></SternSection>
      <SternSection title="Today's schedule" className="stern-overview-schedule" flush note="America/New_York" testId="stern-overview-schedule"><div data-testid="stern-schedule-list">{!data?<SkeletonRows/>:!data.schedule.length?<EmptyState icon={<CalendarDays/>} title="No events scheduled today" hint="Add course meeting times or sync your calendar."/>:data.schedule.map(e=><div className="stern-schedule-row" key={e.key}><time className="stern-mono" dateTime={e.startAt}>{e.startAt.length===10?'All day':clock(e.startAt)}</time><span>{e.href?<Link data-testid={`stern-schedule-open-${e.key}`} href={e.href}>{e.title}</Link>:e.title}</span><small className="stern-muted">{e.location||'Location not set'}</small>{e.prepHref&&<Link data-testid={`stern-schedule-prep-${e.key}`} href={e.prepHref}>Prep</Link>}</div>)}</div></SternSection>
      <SternSection title="Auto-applied today" className="stern-overview-audit" flush testId="stern-overview-audit">{data?<AuditLogRows rows={data.autoAppliedToday} busy={action.busy} undo={batchId=>void action.act({action:'batch.undo',batchId})}/>:<SkeletonRows/>}<p className="stern-memo-line" data-testid="stern-overview-memo">{data?.reminders.lastMemoAt?<>Morning memo sent <time className="stern-mono" dateTime={data.reminders.lastMemoAt}>{clock(data.reminders.lastMemoAt)}</time></>:data?'Morning memo has not been sent today':'Loading memo status…'} <Link data-testid="stern-memo-view" href="/stern/automation#reminders">View</Link></p></SternSection>
    </div>
  </SternPage>;
}
