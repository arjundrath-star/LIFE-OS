...
import { statusLabel, type SternAuditRow } from '@/lib/stern-types';
...
export function useAutomationAction(onSaved?:(response:any)=>void, endpoint='/api/stern/automation') {
  const [busy,setBusy]=useState(false),[message,setMessage]=useState(''),[error,setError]=useState('');
  async function act(body:Record<string,unknown>) {
    setBusy(true);setError('');setMessage('');
    try { const response=await apiPost(endpoint,body);onSaved?.(response);
...
export function ActionNotice({error,message}:{error:string;message:ReactNode}) {return <>{error&&<p className="stern-recruiting-error" role="alert">{error}</p>}{message&&<p className="stern-action-notice" role="status">{message}</p>}</>;}
export function AuditLogRows({rows,busy,undo}:{rows:SternAuditRow[];busy:boolean;undo:(batchId:string)=>void}) {
  ...
  return <><div className="stern-audit-scroll" data-testid="stern-audit-list">{!rows.length?<EmptyState title="No changes recorded" hint="Automatic updates appear here with their source and batch Undo."/>:<table className="stern-audit-table"><thead><tr><th>Time</th><th>Change</th><th>Source</th><th>Evidence</th><th>Batch</th></tr></thead><tbody>{rows.map(row=>{
    ...
    const value=(v:string)=>{const label=/(?:status|state|applied|relationship_type|kind|track|category)$/.test(row.field)?statusLabel(v):v;return label.length>100?`${label.slice(0,100)}…`:label;};
    const anchor=anchors.has(row.batch_id)?undefined:`batch-${row.batch_id}`; anchors.add(row.batch_id);
    return <tr id={anchor} key={row.id} data-component="AuditLogRow" data-testid={`stern-audit-${row.id}`}><td><time className="stern-mono" dateTime={row.created_at}>{dateLabel(row.created_at,true)}</time></td><td><strong>{row.entity_label || `${statusLabel(row.entity_type)} #${row.entity_id}`}</strong><div>{row.field?`${row.field.replace(/_/g,' ')}: `:''}{row.action==='update'?`${value(row.before_value)||'Empty'} → ${value(row.after_value)||'Empty'}`:row.action}</div></td><td><SourceBadge source={row.source}/></td><td>{row.evidence_excerpt?<button className="stern-text-button" data-testid={`stern-audit-evidence-${row.id}`} onClick={()=>setEvidence(row)}>View snippet</button>:<span className="stern-muted">No snippet</span>}</td><td>{row.undone_at?'Undone':canUndo?<button className="stern-text-button" disabled={busy} data-testid={`stern-audit-undo-${row.id}`} onClick={()=>undo(row.batch_id)}>Undo</button>:null}</td></tr>;
