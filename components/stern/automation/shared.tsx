...
export function AuditLogRows({rows,busy,undo}:{rows:SternAuditRow[];busy:boolean;undo:(batchId:string)=>void}) {
  ...
  return <><div className="stern-audit-scroll" data-testid="stern-audit-list">{!rows.length?<EmptyState title="No changes recorded" hint="Automatic updates appear here with their source and batch Undo."/>:<table className="stern-audit-table"><thead><tr><th>Time</th><th>Change</th><th>Source</th><th>Evidence</th><th>Batch</th></tr></thead><tbody>{rows.map(row=>{
    ...
    return <tr key={row.id} data-component="AuditLogRow" data-testid={`stern-audit-${row.id}`}><td><time className="stern-mono" dateTime={row.created_at}>{dateLabel(row.created_at,true)}</time></td><td><strong>{row.entity_label || `${statusLabel(row.entity_type)} #${row.entity_id}`}</strong><div>{row.field?`${row.field.replace(/_/g,' ')}: `:''}{row.action==='update'?`${value(row.before_value)||'Empty'} → ${value(row.after_value)||'Empty'}`:row.action}</div></td><td><SourceBadge source={row.source}/></td><td>{row.evidence_excerpt?<button className="stern-text-button" data-testid={`stern-audit-evidence-${row.id}`} onClick={()=>setEvidence(row)}>View snippet</button>:<span className="stern-muted">No snippet</span>}</td><td>{row.undone_at?'Undone':canUndo?<button className="stern-text-button" disabled={busy} data-testid={`stern-audit-undo-${row.id}`} onClick={()=>undo(row.batch_id)}>Undo</button>:null}</td></tr>;
