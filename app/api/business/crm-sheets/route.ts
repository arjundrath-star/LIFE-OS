...
export async function GET(req: Request) {
  ...
  const limit = Math.min(250, Math.max(1, Number(q.get("limit")) || 200)); const offset = Math.max(0, Number(q.get("offset")) || 0);
  return NextResponse.json(await readCrmSource(source as CrmSourceId, limit, offset));
}
