...
export async function GET(){if(!(await requireUser()))return NextResponse.json({error:"unauthorized"},{status:401});const machines=fleetServiceSummary();return NextResponse.json({machines,history:serviceHistory(),kpis:{total:machines.length,live:machines.filter(m=>m.status==="live"&&m.condition!=="retired").length,needsService:machines.filter(m=>m.condition==="service_required"||m.condition==="out_of_order").length,openIssues:machines.reduce((n,m)=>n+m.open_issues.length,0),verified:machines.filter(m=>m.latest_verified_at).length,unknown:machines.filter(m=>!m.latest_verified_at).length}});}
